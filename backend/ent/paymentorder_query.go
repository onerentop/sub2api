...
func (_q *PaymentOrderQuery) QueryProduct() *ProductQuery {
...
func (_q *PaymentOrderQuery) WithProduct(opts ...func(*ProductQuery)) *PaymentOrderQuery {
	query := (&ProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withProduct; query != nil {
		if err := _q.loadProduct(ctx, query, nodes, nil,
			func(n *PaymentOrder, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PaymentOrderQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *Product)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*PaymentOrder)
	for i := range nodes {
