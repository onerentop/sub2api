...
type antigravityRetryLoopParams struct {
	...
	model          string // 用于 model-based 429 tracking
	c              *gin.Context
	httpUpstream   HTTPUpstream
	settingService *SettingService
	handleError    func(ctx context.Context, prefix string, account *Account, statusCode int, headers http.Header, body []byte, quotaScope AntigravityQuotaScope, model string) // model for 429 tracking
}
	...
	for urlIdx, baseURL := range availableURLs {
		...
		for attempt := 1; attempt <= antigravityMaxRetries; attempt++ {
			...
			if resp.StatusCode == http.StatusTooManyRequests {
				...
				// 重试用尽，标记账户限流
				p.handleError(p.ctx, p.prefix, p.account, resp.StatusCode, resp.Header, respBody, p.quotaScope, p.model)
...
func (s *AntigravityGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
	...
	result, err := antigravityRetryLoop(antigravityRetryLoopParams{
		...
		model:          originalModel, // 用于 model-based 429 tracking
		c:              c,
