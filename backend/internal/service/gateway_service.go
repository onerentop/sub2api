	...
	if len(routingAccountIDs) == 0 && sessionHash != "" && s.cache != nil {
		...
		if err == nil && accountID > 0 && !isExcluded(accountID) {
			...
			if ok {
				...
				if !clearSticky && model429Available && s.isAccountInGroup(account, groupID) &&
		...
		if err == nil && accountID > 0 {
			if _, excluded := excludedIDs[accountID]; !excluded {
				...
				if err == nil {
					...
					if !clearSticky && s.isAccountInGroup(account, groupID) && account.Platform == platform && account.IsSchedulableForModel(requestedModel) && (requestedModel == "" || s.isModelSupportedByAccount(account, requestedModel)) && (requestedModel == "" || s.model429Tracker.IsAccountAvailableForModel(account.ID, requestedModel)) {
						if err := s.cache.RefreshSessionTTL(ctx, derefGroupID(groupID), sessionHash, stickySessionTTL); err != nil {
							log.Printf("refresh session ttl failed: session=%s err=%v", sessionHash, err)
						}
						return account, nil
		...
		if err == nil && accountID > 0 {
			if _, excluded := excludedIDs[accountID]; !excluded {
				...
				if err == nil {
					...
					if !clearSticky && s.isAccountInGroup(account, groupID) && account.IsSchedulableForModel(requestedModel) && (requestedModel == "" || s.isModelSupportedByAccount(account, requestedModel)) && (requestedModel == "" || s.model429Tracker.IsAccountAvailableForModel(account.ID, requestedModel)) {
						if account.Platform == nativePlatform || (account.Platform == PlatformAntigravity && account.IsMixedSchedulingEnabled()) {
							if err := s.cache.RefreshSessionTTL(ctx, derefGroupID(groupID), sessionHash, stickySessionTTL); err != nil {
								log.Printf("refresh session ttl failed: session=%s err=%v", sessionHash, err)
							}
