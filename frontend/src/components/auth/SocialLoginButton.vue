...
import { startSocialLogin } from '@/api/social'
...
const buttonClass = computed(() => {
  switch (props.provider) {
    case 'google':
      return 'btn-outline hover:bg-gray-50 dark:hover:bg-dark-700'
    case 'github':
      return 'btn-neutral'
    default:
...
async function handleClick() {
  ...
  try {
    ...
    const result = await startSocialLogin(props.provider, redirectTo)

    // 存储 session_id 到 localStorage 以便回调时使用
    localStorage.setItem('social_oauth_session', JSON.stringify({
